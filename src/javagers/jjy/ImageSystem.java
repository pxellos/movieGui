...
//그리고, ImagePanel을 다시 그린다.
	...
	RealMain realMain;
	LoadActionListener(ImageSystem is, ImagePanel ip, RealMain main){
		this.is = is; this.ip = ip; this.realMain = main;
	}
...
class SaveActionListener implements ActionListener{
	FileDialog fileDialog; ImageSystem is; 
	BufferedOutputStream bos;//파일처리에 사용하는 객체
	RealMain main;
	SaveActionListener(ImageSystem is,RealMain main){
		this.is = is; 
		this.main = main;
	}
	static String path;
	...
	public void actionPerformed(ActionEvent e) {
		fileDialog = new FileDialog(main,"이미지 파일 저장",
										FileDialog.SAVE);
...
public class ImageSystem extends Panel {
	Button load,save; 
	TextField text; Panel southPanel;
	ImagePanel imagePanel; 
	RealMain main;

	public ImageSystem(RealMain main) {
//		super(str);
		this.setLayout(new BorderLayout());
		load=new Button("이미지 불러오기");save= new Button("Image Save");
		imagePanel = new ImagePanel();
		load.addActionListener(new LoadActionListener(this,imagePanel,main));
		save.addActionListener(new SaveActionListener(this,main));
